...
program.version(pkg.version)
  ...
  .option('--ui <interface>', 'The UI interface to use. (default: cli)');
...
exports.run = function(){
	...
	var ui = program.ui || "cli";

	var events = options.events = serverEvents();
	var ifaces = [require("./interfaces/" + ui)];
	...
	if(program.develop) {
		...
		if(!fs.existsSync(stealToolsPath)) {
			events.emit("error", 'live-reload not available: ' +
	...
	server.on('error', function(e) {
		if(e.code === 'EADDRINUSE') {
            events.emit("error", 'ERROR: Can not start done-serve on port ' + port +
				'.\nAnother application is already using it.');
		} else {
			events.emit("error", e);
			events.emit("error", e.stack);
		}
	});
